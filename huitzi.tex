...
The Huitzi imager has an Andor iXon Ultra 888 unit with an e2v CCD201-20 electron-multiplying CCD detector. The detector has $1024\times1024$ pixels with a pixel scale of 0.68 arcsec and a field of 11.6 arcmin. The detector operates as a frame-transfer CCD and can be read through either a conventional amplifier or the electron-multiplying amplifier at a variety of speeds. The detector has standard silicon and the e2v midband coating (which in Andor's terminology makes it a “BV” device).
...
\begin{itemize}
    ...
    \item 470/10, 515/10, 640/10 continuum; and
    \item 656/3 H$\alpha$.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Move the filter wheel by hand to expose the correct filter. See Figure~\ref{figure:huitzi-filter-wheel-slots}. The 8 filter slots are labelled from 0 to 7. At the time of writing, the slots are filled as follows:
    \begin{enumerate}
        \item[0:] $g$
        \item[1:] $r$
        \item[2:] $i$
