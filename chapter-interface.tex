...
The top-level and the documentation are available to computers on the mountain-top, Instituto de Astronomía CU, and Instituto de Astronomía Ensenada networks.

The interface is only directly available to computers on the mountain-top network. 
...
\label{section:interface-local-access}

The interface can be accessed locally by OAN technical staff using the Raspberry Pi in the shed.

If you need to log into the Raspberry Pi, use the “{\projectaccount}” account with password “{\projectaccount}”. After this, open Chromium and follow the instructions above for access to the web site.

\section{Main Page}

\begin{figure*}
\begin{center}
\resizebox{\linewidth}{!}{\begin{labeled}{\includegraphics[width=0.5\linewidth]{figures/interface-main-page}}
...
\begin{itemize}
\item
Enable. Enabled the supervisor. This permits the supervisor to open and close according to the weather and sky state. (In this context, the "supervisor" is the part of the control system that is responsible for taking the decision to open or close the telescope.)
\item
